...
const TechStack = () => {
  ...
  useEffect(() => {
    const handleViewportState = () => {
      ...
      // Activate and render only when the section is actually near/in viewport.
      const inView =
  ...
  const activeMaterials = materials.length ? materials : [fallbackMaterial];
          ...
          <Physics gravity={[0, 0, 0]}>
            ...
            {spheres.map((props, i) => (
              <SphereGeo
                ...
                {...props}
                material={
                  activeMaterials[Math.floor(Math.random() * activeMaterials.length)]
                }
                isActive={isActive}
              />
            ))}
